            ...
            VStack(alignment: .leading, spacing: 16) {
                ...
                settingsGroup {
                    settingsRow {
                        ...
                        .labelsHidden()
                        .frame(width: 240)
                        .accessibilityLabel("Interface material")
                    }
                ...
                settingsGroup {
                    ...
                    settingsRow {
                        VStack(alignment: .leading, spacing: 2) {
                            Text("Volume Step")
                                .font(.system(size: 13))
                            Text("Used by step buttons, arrow keys, VoiceOver, and global hotkeys.")
                                .font(.system(size: 10))
                                .foregroundColor(.secondary)
                        }
                        Spacer()
                        Picker("", selection: $deviceState.volumeStep) {
                            ForEach(VolumeScale.allowedSteps, id: \.self) { step in
                                Text("\(step) dB").tag(step)
                            }
                        }
                        .frame(width: 120)
                        .accessibilityLabel("Volume adjustment step")
                    }
                    ...
                    settingsRow {
                        ...
                        Picker("", selection: $deviceState.nightModeMax) {
                            ForEach(VolumeScale.allowedNightModeCaps, id: \.self) { cap in
                                Text("-\(cap) dB").tag(cap)
                            }
                        }
                        .frame(width: 120)
                        .accessibilityLabel("Night mode volume cap")
                    }
                }
                    ...
                    ForEach(Array(MIDIAction.allCases.enumerated()), id: \.element.rawValue) { index, action in
                        ...
                        settingsRow {
                            ...
                            if let mapping = midiManager.getMapping(for: action) {
                                ...
                                .buttonStyle(.plain)
                                .accessibilityLabel("Remove \(action.displayName) MIDI mapping")
                            } else {
    ...
    var body: some View {
        HStack(spacing: 4) {
            ...
            if currentBinding != nil && !isRecording {
                ...
                .buttonStyle(.plain)
                .accessibilityLabel("Remove \(action.rawValue) shortcut")
            }
        }
