...
struct MenuBarView: View {
    ...
    private func adjustAccessibilityVolume(_ direction: AccessibilityAdjustmentDirection) {
        switch direction {
        case .increment: adjustVolumeSlider(by: 1)
        case .decrement: adjustVolumeSlider(by: -1)
    ...
    private func adjustKeyboardVolume(_ direction: MoveCommandDirection) {
        switch direction {
        case .up, .right: adjustVolumeSlider(by: 1)
        case .down, .left: adjustVolumeSlider(by: -1)
        default: break
        }
    }
