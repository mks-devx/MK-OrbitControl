...
struct MIDIMapping: Codable, Equatable, Identifiable {
    ...
    var isValid: Bool {
        (0...15).contains(channel) && (0...127).contains(cc)
    }

    static func reconciled(_ mappings: [MIDIMapping]) -> [MIDIMapping] {
        mappings.reduce(into: []) { result, mapping in
            guard mapping.isValid else { return }
            result.removeAll {
                $0.action == mapping.action
                    || ($0.channel == mapping.channel && $0.cc == mapping.cc)
            }
...
class MIDIManager: ObservableObject {
    ...
    private func connectAllSources() {
        let availableSources = Set((0..<MIDIGetNumberOfSources()).compactMap { index -> MIDIEndpointRef? in
            let source = MIDIGetSource(index)
            return source == 0 ? nil : source
        })

        for source in connectedSources.subtracting(availableSources) {
            MIDIPortDisconnectSource(inputPort, source)
        }
        for source in availableSources.subtracting(connectedSources) {
    ...
    private func saveMappingAndStopLearn(_ mapping: MIDIMapping) {
        mappings = MIDIMapping.reconciled(mappings + [mapping])
        isLearning = false
        learningAction = nil
        persistMappings()
    }
    ...
    private func loadMappings() {
        ...
        mappings = MIDIMapping.reconciled(m)
    }
