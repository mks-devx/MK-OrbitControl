...
struct HotkeyBinding: Codable, Equatable {
    ...
    static func reconciled(_ bindings: [HotkeyBinding]) -> [HotkeyBinding] {
        bindings.reduce(into: []) { result, binding in
            result.removeAll {
                $0.action == binding.action
...
final class HotkeyManager {
    ...
    func save(_ binding: HotkeyBinding) {
        let all = HotkeyBinding.reconciled(loadAll() + [binding])
        persist(all)
        registerAll()
    }
    ...
    private func loadAll() -> [HotkeyBinding] {
        ...
        return HotkeyBinding.reconciled(bindings)
    }
    ...
    private func executeAction(_ action: HotkeyAction) {
        ...
        if action.isMute {
            ...
            let vol = deviceState.channels[channel]?.volume ?? VolumeScale.maximumRaw
            VolumeHUD.shared.show(volume: vol, muted: newMute, channel: channel.label)
        } else if action.isVolumeUp {
            let newVol = VolumeScale.adjustedRaw(
                deviceState.channels[channel]?.volume,
                by: -deviceState.volumeStep
            )
